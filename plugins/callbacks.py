...
async def select_resolution(client, callback_query):
    ...
    d_buttons = [InlineKeyboardButton(
        text="Скачать", callback_data=f"download_{video_id}:{cb_resolution}:{c_type}")]
...
async def select_file_type(client, callback_query):
    ...
    formatted_text = callback_query.message.caption
    t_buttons = [
        InlineKeyboardButton(text="Видос ✔" if cb_type == 'video' else "Video",
                             callback_data=f"type_{video_id}:video"),
        InlineKeyboardButton(text="Звучёк ✔" if cb_type == 'audio' else "Audio",
    ...
    video_id, c_resolution, c_type = cb_d_button_data.split(':')
    d_buttons = [InlineKeyboardButton(
        text="Скачать", callback_data=f"download_{video_id}:{c_resolution}:{cb_type}")]
...
async def download(client, callback_query):
    ...
    if c_resolution == 'n' and c_type == 'n':
        ...
        if video is not None:
            ...
            return await callback_query.answer("Пж выбери другое качество ато это щас не даступна", show_alert=True)
        ...
        if audio_streams is not None:
            ...
            return await callback_query.answer("Аудио файл не доступен для этой ссылки!", show_alert=True)
        # Send the downloaded video to the user
...
async def pl_select_file_type(client, callback_query):
    ...
    video_id, c_resolution, c_type = cb_d_button_data.split(':')
    d_buttons = [InlineKeyboardButton(
        text="Скачать", callback_data=f"pl_download_{playlist_id}:{c_resolution}:{cb_type}")]
...
async def pl_download(client, callback_query):
    ...
    final_text = f"Плейлист <b>{playlist_title}</b> скачан и загружен в чат! \n\n<b>Работает благодаря: @WerhesDev и <a href='https://t.me/OKAKDLPBot''>@OKAKDLPBot</a></b>."
    ...
    try:
        for video in playlist.videos:
            ...
            author = video.author
            confirm_text = f"Видос <b>{title} скачивается ...</b>"
            formatted_text = f"<b>{title}</b>\n\n{description[:300]}{'...' if len(description) > 300 else ''}"
            try:
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "ААААААААА БОЖЕЕ ПАМАГИТЕ ПАДЕШКАААААААА":
        buttons = [
            [
                InlineKeyboardButton("что это такое", callback_data='about_cb'),
                InlineKeyboardButton("⚡Нет я хочу ливнуть | назад", callback_data='start_cb'),
                InlineKeyboardButton("Поддержка", callback_data='help_cb'),
            ],
            [
                InlineKeyboardButton(
                    "👨🏼‍💻Разраб", url='https://t.me/WerhesDev')        
            ],
            [
        ...
        buttons = [
            [
                InlineKeyboardButton("🔮помогите", callback_data='help_cb'),
                InlineKeyboardButton("⚡назад", callback_data='start_cb')
            ],
            [
                InlineKeyboardButton(
                    "👨🏼‍💻Разраб", url='https://t.me/WerhesDev')
