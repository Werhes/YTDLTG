...
async def handle_youtube_link(bot, message):
    ...
    try:
        ...
        t_buttons.append(InlineKeyboardButton(
            text="Видос", callback_data=f"type_{video_id}:video"))
        t_buttons.append(InlineKeyboardButton(
            text="Звучёк", callback_data=f"type_{video_id}:audio"))

        # add a 'download' button in another new row
        d_buttons = [InlineKeyboardButton(
            text="скачать", callback_data=f"download_{video_id}:n:n")]
...
async def handle_youtube_playlist_link(bot, message):
    try:
        ...
        match = re.search(r"Видосы=([A-Za-z0-9_-]+)", playlist_url)
        if match:
        ...
        t_buttons = []
        t_buttons.append(InlineKeyboardButton(
            text="Видос", callback_data=f"pl_type_{playlist_id}:video"))
        t_buttons.append(InlineKeyboardButton(
            text="Звучёк", callback_data=f"pl_type_{playlist_id}:audio"))

        d_buttons = [InlineKeyboardButton(
            text="скачать", callback_data=f"pl_download_{playlist_id}:n:n")]
        ...
        await bot.send_message(message.chat.id, f"Ошибочка вылезла: {e}")
